...
- **Các mục CÒN LẠI trong `index.html` (trang chủ cũ) vẫn trỏ Apps Script, CHƯA có bản Supabase** — cần làm tiếp theo đúng tinh thần "bỏ hẳn Google" người dùng yêu cầu: **IPQC** (`Ipqc.html`, màn kiểm tra viên nộp kết quả — RPC `duc_submit_ipqc_check`/`duc_request_ipqc_check` đã có sẵn từ step 4, chỉ thiếu giao diện), **QC Manager** (`QcManager.html`, giám sát QC + tiêu chuẩn IPQC + NCP — 1 phần RPC đã có, luồng NCP đầy đủ cũng chưa có UI), **Tồn kho NVL** (Phase 5, CHƯA khảo sát/migrate database, hiện là 1 trang GitHub Pages RIÊNG — `https://dangnaf-toyo.github.io/Ton-kho-NVL/` — không phải Apps Script như 2 mục kia, cần kiểm tra lại xem trang đó đã dùng Supabase hay vẫn nguồn khác trước khi kết luận có cần làm lại không).
