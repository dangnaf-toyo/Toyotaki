...
const SOURCE_ID = "16w0e8XQn354S1MC8PdcvYvis81w4clmH60P0KZCAx08";

function onOpen() {

  SpreadsheetApp.getUi()
  .createMenu("QC MENU")
  .addItem("🔄 Cập nhật thống kê","CapNhatThongKe")
  .addToUi();
...
function CapNhatThongKe(){
  ...
  const sheet = target.getSheetByName("BatThuong_Thang");
  ...
  source.getSheets().forEach(function(ws){

    if(!ws.getName().startsWith("W")) return;
    ...
    for(var r=1;r<values.length;r++){
      ...
      var ngay = row[1];       // Cột B = Ngày
      var noi = row[2];        // Cột C = Nơi phát sinh
      var status = row[12];    // Cột M = Trạng thái

      if(!(ngay instanceof Date)) continue;

      var thang = ngay.getMonth()+1;

      if(noi=="Khách hàng"){

        result[thang].KH++;

        if(String(status).toUpperCase()=="CLOSE"){

          result[thang].KH_Close++;
  ...
  for(var m=1;m<=12;m++){
    ...
    sheet.getRange(m+1,2).setValue(result[m].KH);

    sheet.getRange(m+1,3).setValue(result[m].KH_Close);

    sheet.getRange(m+1,4).setValue(result[m].NB);

    sheet.getRange(m+1,5).setValue(result[m].NB_Close);
